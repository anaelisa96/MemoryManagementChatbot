...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        ////
                        /*
                            My Comment - Task 3: Adapt creation of Graphnode node pointer because now is a smart unique pointer
                        */
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if (((*it).get())->GetNumberOfParents() == 0)
        {
            ...
            {
                /*
                    My Comment - Task 3: Adapt rootNode assignment because it must be a raw pointer
                */
